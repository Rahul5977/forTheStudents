                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {sessions.map((s) => (
                      <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, background: 'var(--color-bg)', borderRadius: 10, padding: '7px 10px', flexWrap: 'wrap' }}>
                        ...
                        {s.meetingUrl && <a href={s.meetingUrl} target="_blank" rel="noreferrer" title={s.meetingProvider === 'stub' ? 'Placeholder link — owner connects Google Calendar to make it real' : 'Google Meet'} style={{ fontSize: 12.5 }}>🎥 Meet{s.meetingProvider === 'stub' ? ' (placeholder)' : ''}</a>}
