// ════════════════════════════════════════════════════════════════════════════
// DEV-ONLY local server. Runs the SAME Hono app the Lambda runs, on your laptop,
// backed by DynamoDB Local. It fakes the ONE thing the cloud gives us: a verified
// JWT. Lets the real frontend do login -> token -> save choice list against a real
// DB with zero AWS. The doctor endpoint reads the catalog snapshot — seed it first
// with `pnpm --filter @sc/catalog seed`.
//
// Start:  pnpm dev:db                     (DynamoDB Local)
//         pnpm --filter @sc/catalog seed   (once, so doctor has cutoffs)
//         pnpm --filter @sc/planner dev
//
...
import { ensurePlannerTable } from './local-table';
...
async function main() {
  ...
  console.log(`\n  planner DEV server  →  http://localhost:${PORT}`);
  // eslint-disable-next-line no-console
  console.log('  Try: POST /dev/login {email} -> token, then PUT /choice-list {items:[..]} with Bearer token\n');
}
