...
- [x] **Session lifecycle** — `POST /sessions/:id/join` (asserts participant + paid + time-window, mints a video token = **SFU stub, TODO owner**) → LIVE; `/end` → ENDED; `/rate` → RATED (emits `session.rated`); `GET /sessions` (my sessions, both roles); `GET /bookings/:id` (booking + ledger)
