...
3. **Phase 12 — College Data Hub** is being built (`docs/college-data-hub.md`): 12a foundations + the 6-institute pilot collection are done (2026-09-16); next is owner review of `data/colleges/coverage.json` before the 10-institute 12b batches.
...
- [x] Seat-matrix step: JoSAA 2026 seat matrix → `seat-matrix.json` for all 86 roster ids (2026-09-15)
- [x] **Pilot collection** — 6 institutes (`pilot: true`) via the Workflow tool (4 collectors + 1 verifier each) → `hub:validate` clean (2026-09-16; the run that started 2026-09-14 hit a usage-limit 429 after 1/30 agents, resumed as a fresh Workflow run scoped to the 3 unfinished institutes — nit-warangal, iiit-allahabad, iiitdm-jabalpur — plus fixed pre-existing schema-length overflows in `nit-trichy/{fees,placements}.json` and `_meta.json` shape mismatches)
...
- **2026-09-16** — **Phase 12 pilot collection complete: 6/6 institutes.** Finished the 3 pilot institutes left incomplete by the 2026-09-14 run (which hit a usage-limit 429 after 1/30 agents): NIT Warangal (placements/fees, campus-life/hostels/social, alumni/faqs), IIIT Allahabad (academics/rankings + B/C/D), IIITDM Jabalpur (full A–D) — via a fresh Workflow run scoped to just the missing packets per institute, each followed by an adversarial verifier pass writing `_meta.json`. Also fixed pre-existing `nit-trichy/{fees,placements}.json` schema overflows (source titles >200 chars, scholarship fields >300 chars) and `_meta.json` shape mismatches (missing `packets`/`verifierReport` wrapper) uncovered while getting the whole pilot to `hub:validate` clean. All 6 pilot institutes (IIT Bombay, IIT Madras, NIT Trichy, NIT Warangal, IIIT Allahabad, IIITDM Jabalpur) now have full 11-section data; `hub:build` produces a 693 KB bundle. Next: owner reviews `data/colleges/coverage.json` + spot-checks a couple of source URLs before the 12b batches (remaining ~80 institutes).
