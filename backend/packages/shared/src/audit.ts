// Append-only admin audit trail (Audit table, architecture.md §6.2). Shared (Phase 11) because
// more than one service must write to it: admin-ops (moderation/broadcast), auth-identity
// (superadmin bootstrap, admin promotion/demotion) and marketplace (verification decisions,
// document access). Every service that writes gets TABLE_AUDIT + a write grant in infra.
...
// admin (Query, no scan). A cross-admin/by-entity view (`GSI1: entity`) is deferred.
import { PutCommand, QueryCommand } from '@aws-sdk/lib-dynamodb';
import { getEnv } from '@sc/config';
import { ddb, key } from './ddb';
